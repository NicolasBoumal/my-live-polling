...
1. Allow authentication with Google and also as Anonymous user.
...
6. In each file, look for `firebaseConfig`: copy your data there (provided by Firebase, under Settings -> General).

Usage should be self explanatory. In a nutshell:

* Open the `admin.html` page on your phone.
* Have participants opn the `index.html` page on their devices.
* Open `dashboard.html` on your lecture screen (or integrate with the .js/.css files, see `overlay_example.html` -- perfect for a revealjs presentation!).
